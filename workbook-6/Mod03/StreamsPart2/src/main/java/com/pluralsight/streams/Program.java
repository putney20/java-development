...
public class Program {
    ...
    private static void searchByFirstName(ArrayList<Person> person) {
        ...
        List<Person> matchingFirstName = person.stream().filter(first -> first.equals(input)).toList();
    ...
    private static void searchByLastName(ArrayList<Person> person) {
        ...
        Scanner scanner = new Scanner(System.in);
        String input = scanner.nextLine();
        List<Person> matchingLasttName = person.stream().filter(last -> last.equals(input)).collect(Collectors.toList());
        for (Person p : matchingLasttName) {
